...
export default function EmployeePerformanceReport() {
 ...
 console.log("Employeeselectdata", Employeeselectdata);
  ...
  function fetchDailyStatusReport() {
    ...
    axios
      ...
      .then((response) => {
        ...
        totalqnty(response.data["Total Qnty"]);
        totalcost(response.data["Total Cost"]);
        totalamount(response.data["Total Amount"]);
        totalmargin(response.data["Total margin"]);
        totaldelivery(response.data["Total Delivery"]);
        totalCom(response.data["Total Comm"]);
  ...
  const exportPDFHandler = () => {
    ...
    const columnWidths = [24, 100, 25, 25,25, 25, 25, 25];
    ...
    const handlePagination = () => {
      ...
      while (currentPageIndex * rowsPerPage < rows.length) {
        ...
        doc.setFont("verdana-regular", "normal");
        doc.setFontSize(10);
        doc.text(`${typefilter}`, labelsX + 25, labelsY); // Draw the value next to the label

      
        doc.setFont("verdana", "bold");
        doc.setFontSize(10);
        doc.text(`Commision :`, labelsX + 180, labelsY + 4.3); // Draw bold label
        doc.setFont("verdana-regular", "normal");
        doc.setFontSize(10);
        doc.text(`${mobileNumber}`, labelsX + 205, labelsY + 4.3); // Draw the value next to the label
    ...
    // Save the PDF files
    doc.save(`EmployeePerformanceReport As On ${date}.pdf`);
  };
  ...
  const handleDownloadCSV = async () => {
    ...
    // Set column widths
    [10, 45, 12, 12,12, 12, 12, 12].forEach((width, index) => {
  ...
  "",
    ...
    saveAs(blob, `EmployeePerformanceReport As On ${currentdate}.xlsx`);
  };
...
/>
 ...
 <div
            ...
            >
              <span className="mobileledger_total">{formatValue(totalamount)}</span>
            </div>
            <div
              style={{
                ...seventhColWidth,
