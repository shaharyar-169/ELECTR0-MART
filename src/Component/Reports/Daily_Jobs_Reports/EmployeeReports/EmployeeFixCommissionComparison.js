...
export default function EmployeeFixCommissionComparison() {
  ...
  function fetchDailyStatusReport() {
    ...
    const apiUrl = apiLinks + "/EmployeeFixCommissionComparison.php";
    setIsLoading(true);
  ...
  const exportPDFHandler = () => {
    ...
    rows.push([String(formatValue(tableData.length.toLocaleString())), "",  String(formatValue(totaldebit)), String(formatValue(totalcredit)), String(formatValue(Commission))]);

    // Define table column headers and individual column widths

    const headers = ["Code", "Employee",  "Qnty", "Amount", "Fix Comm"];
 ...
 doc.setFont("verdana-regular", "normal");
    ...
    doc.setFontSize(12);
        addTitle(
          `Employee Fix Commission Comparison Report From ${fromInputDate} To ${toInputDate}`,
          "",
          "",
          pageNumber,
          startY,
        ...
        doc.setFont("verdana", "bold"); // Set font to bold
        doc.setFontSize(10);
        doc.text(`Capacity :`, labelsX + 120, labelsY + 4.3); // Draw bold label
        doc.setFont("verdana-regular", "normal"); // Set font to bold
        doc.setFontSize(10);
        doc.text(`${typeText}`, labelsX + 145, labelsY + 4.3); // Draw the value next to the label
    ...
    // Save the PDF files
    doc.save(`EmployeeFixCommissionComparison As On ${date}.pdf`);
  };
  ...
  cell.alignment = { horizontal: "center" };
    ...
    // Set column widths
    [10, 45, 10, 12,12].forEach((width, index) => {
    ...
    saveAs(blob, `EmployeeFixCommissioComparison As On ${currentdate}.xlsx`);
  };
  ...
  return (
    <>
      ...
      <div style={contentStyle}>
        ...
        >
          <NavComponent textdata="Employee Fix Commission Comparison Report" />
