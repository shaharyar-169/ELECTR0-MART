...
export default function InstallarList() {
  ...
  const exportPDFHandler = () => {
     ...
     // Create a new jsPDF instance with landscape orientation
     const doc = new jsPDF({ orientation: "portrait" }); // Fixed typo: "potraite" -> "portrait"
     ...
     // Define table column headers and individual column widths
     const headers = ["Code", "Installar", "Sts","Mobile", "City"];
     const columnWidths = [18, 90, 14,30,40];
  ...
  doc.setFont("verdana-regular", "normal");
   ...
   const handleDownloadCSV = async () => {
     ...
     const columnAlignments = ["center", "left", "center","center","left"];
    ...
    //      transectionType === "A" ? "ACTIVE" : "ALL";
     ...
     typeAndStoreRow3.eachCell((cell, colIndex) => {
       cell.font = { name: "CustomFont", size: 10, bold: [3].includes(colIndex) };
       cell.alignment = { horizontal: "left", vertical: "middle" };
     });
  ...
  item.Code,
     ...
     // Column widths
     [6, 40,6,12, 20].forEach((width, index) => {
       worksheet.getColumn(index + 1).width = width;
     });
