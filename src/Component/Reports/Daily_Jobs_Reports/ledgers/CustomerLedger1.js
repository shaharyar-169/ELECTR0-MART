    ...
    axios
      ...
      .then((response) => {
        ...
        if (response.data && Array.isArray(response.data.Header)) {
          ...
          // Calculate other totals (you may need to adjust these based on your actual data)
          let totalDebit = 0;
          let totalCredit = 0;
          let totalQnty = 0;

          response.data.Header.forEach((item) => {
            if (item.debit) totalDebit += parseFloat(item.debit) || 0;
            if (item.credit) totalCredit += parseFloat(item.credit) || 0;
            if (item.titmqnt) totalQnty += parseFloat(item.titmqnt) || 0;
          });
          ...
          if (response.data.amt005 !== undefined) {
            setClosingBalance(response.data.amt005);
          }
