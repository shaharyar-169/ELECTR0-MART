...
// ================= END BALANCE AGING ROW =================
    ...
    const addTableRows = (startX, startY, startIndex, endIndex) => {
      ...
      for (let i = startIndex; i < endIndex; i++) {
        ...
        if (isOddRow) {
          doc.setFillColor(230); // Light background color
          doc.rect(
            startX,
...
if (isTotalRow) {
  ...
  continue; // 🚨 skip normal total row rendering
    ...
    const handlePagination = () => {
      ...
      while (currentPageIndex * rowsPerPage < rows.length) {
        ...
        addTitle(
          `Customer Progress Report From: ${toInputDate}`,
          pageNumber,
          startY,
          12
        );
  ...
  const handleDownloadCSV = async () => {
    ...
    worksheet.mergeCells(
      `A${companyRow.number}:${String.fromCharCode(64 + numColumns - 1)}${
        companyRow.number
      }`
    );
    ...
    worksheet.mergeCells(
      `A${storeListRow.number}:${String.fromCharCode(64 + numColumns - 1)}${
        storeListRow.number
      }`
    );
    ...
    const totalRow = worksheet.addRow([
      formatValue(amt1),
     `${formatValue(amt2)} | ${formatValue(amt3)}`, // Add spacing
     `${formatValue(amt4)} | ${formatValue(amt5)}`, // Add spacing
      amt6,
      amt7,
    ...
    // Set column widths
    [11, 15, 15, 15, 15].forEach((width, index) => {
      ...
      <div style={contentStyle}>
        ...
        >
          ...
          >
            ...
            >
              <span className="mobileledger_total">{amt1 }</span>
            </div>
            ...
            >
              <span className="mobileledger_total">{formatValue(amt2)}</span>
              ...
              <span className="mobileledger_total">{formatValue(amt3)}</span>
            </div>
            ...
            >
              <span className="mobileledger_total">{formatValue(amt4)}</span>
              ...
              <span className="mobileledger_total">{formatValue(amt5)}</span>
            </div>
            <div
              style={{
                //  width: "43px",
            ...
            >
              <span className="mobileledger_total">{formatValue(amt6)}</span>
            </div>
            <div
              style={{
                //  width: "43px",
            ...
            >
              <span className="mobileledger_total">{formatValue(amt7)}</span>
            </div>
          </div>
