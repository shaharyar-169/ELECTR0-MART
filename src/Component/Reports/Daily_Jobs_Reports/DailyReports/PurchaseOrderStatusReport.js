...
export default function PurchaseOrderStatusReport() {
  ...
  function fetchDailyStatusReport() {
    ...
    const apiUrl = apiLinks + "/PurchaseOrderStatusReport.php";
    setIsLoading(true);
    ...
    axios
      ...
      .then((response) => {
        setIsLoading(false);
         ...
         settotalopening(response.data["Total Qnty"]);
        settotaldebit(response.data["Total Receive"]);
        settotalcredit(response.data["Total Balance"]);
  ...
  const exportPDFHandler = () => {
    ...
    const rows = tableData.map((item) => [
      ...
      item.Date,
      item.Supplier,
    formatValue(item.OrdQnty),
      formatValue(item.RecQnty),
    ...
    const columnWidths = [17, 24,110, 25, 25, 25,25];
    ...
    const addTableRows = (startX, startY, startIndex, endIndex) => {
      ...
      for (let i = startIndex; i < endIndex; i++) {
        ...
        row.forEach((cell, cellIndex) => {
          ...
          } else if (cellIndex === 3 || cellIndex === 4 || cellIndex === 5 || cellIndex === 6 ) {
            const rightAlignX = startX + columnWidths[cellIndex] - 2;
            doc.text(cellValue, rightAlignX, cellY, {
              align: "right",
              baseline: "middle",
    ...
    const handlePagination = () => {
      ...
      while (currentPageIndex * rowsPerPage < rows.length) {
        ...
        doc.setFontSize(10);
        addTitle(
          `Purchase order Status Report From ${fromInputDate} To ${toInputDate}`,
          "",
          "",
          pageNumber,
          startY,
    ...
    // Save the PDF files
    doc.save(`PurchaseOrderStatusReport As On ${date}.pdf`);
  };
...
worksheet.mergeCells(
  `F${typeAndStoreRow3.number}:G${typeAndStoreRow3.number}`
);
...
typeAndStoreRow3.eachCell((cell, colIndex) => {
  cell.font = {
    name: "CustomFont",
    size: 10,
    bold: colIndex === 5, // SEARCH label bold
  };

  // 🔹 Different alignment for label & value
  if (colIndex === 5) {
    // SEARCH :
    ...
    // Set column widths
    [8, 10, 45, 12, 12,12,12].forEach((width, index) => {
    ...
    saveAs(blob, `PurchaseOrderStatusReport As On ${currentdate}.xlsx`);
  };
  ...
  return (
    <>
      ...
      <div style={contentStyle}>
        ...
        >
          <NavComponent textdata="Purchase Order Status Report" />
              ...
              >
                ...
                >
                  <input
                    ...
                    onKeyDown={(e) => handleToKeyPress(e, input5Ref)}
                    id="toDatePicker"
                    autoComplete="off"
                    placeholder="dd-mm-yyyy"
                ...
                >
                  ...
                  >
                    <td
                      ...
                      style={firstColWidth}
                      //   onClick={() => handleSorting("DMD")}
                    >
                      Ord No
                      {/* {" "}
                      <i
                        className="fa-solid fa-caret-down caretIconStyle"
                        style={getIconStyle("DMD")}
                      ></i> */}
                    </td>
                    <td
                      className="border-dark"
                    ...
                    <td
                      className="border-dark"
                      style={forthColWidth}
                      //   onClick={() => handleSorting("JobNo")}
                    >
                      OrdQnty
                      {/* {" "}
                      <i
                        className="fa-solid fa-caret-down caretIconStyle"
                        style={getIconStyle("JobNo")}
                      ></i> */}
                    </td>
