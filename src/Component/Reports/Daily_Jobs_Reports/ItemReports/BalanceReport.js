...
export default function BalanceReport() {
  ...
  function fetchDailyStatusReport() {
    ...
    const apiUrl = apiLinks + "/BalanceReport.php";
    setIsLoading(true);
    const formData = new URLSearchParams({
      FIntDat: fromInputDate,
      FFnlDat: toInputDate,
      FSchTxt: searchQuery,
      FAccCod: Companyselectdata,
    ...
    axios
      ...
      .then((response) => {
        ...
        settotalClosing(response.data["ClosingBal "]);
  ...
  const options = GetCompany.map((item) => ({
    value: item.tacccod,
    label: `${item.tacccod}-${item.taccdsc}`,
  }));
   ...
   const exportPDFHandler = () => {
      ...
      rows.push([
        ...
        String(formatValue(totalOpening)),
           ...
           String(formatValue(totalClosing)),
      ]);
      ...
      const columnWidths = [28, 110 ,  35, 35, 35, 35];
  ...
  { align: "right" },
        ...
        while (currentPageIndex * rowsPerPage < rows.length) {
          ...
          doc.setFont("verdana-regular", "normal");
          addTitle(
            `Balance Report From ${fromInputDate} To ${toInputDate}`,
            "",
            "",
            pageNumber,
            startY,
          ...
          doc.text(`${typeItem}`, labelsX + 25, labelsY); // Draw the value next to the label
            ...
            doc.setFont("verdana-regular", "normal");
            doc.setFontSize(10);
            doc.text(`${search}`, labelsX + 200, labelsY); // Draw the value next to the label
          }
  
          startY += 12; // Adjust vertical position for the labels
      ...
      // Save the PDF files
      doc.save(`BalanceReport As On ${date}.pdf`);
    };
  ...
  const storeListRow = worksheet.addRow([
    `Balance Report From ${toInputDate} To ${toInputDate}`,
  ]);
  ...
  const headers = ["Code", "Description", "Opening", "Debit", "Credit", "Closing"];
  ...
  tableData.forEach((item) => {
    ...
    const closingNum = toNumber(item.Closing);
    ...
    row.eachCell((cell, colIndex) => {
      ...
      // Apply number formatting for numeric columns (3,4,5,6) – no decimal places
  ...
  // =====================================================================
  // DYNAMIC WIDTH FOR DESCRIPTION COLUMN (unchanged from original)
  // =====================================================================
  ...
  descriptionWidth = Math.max(descriptionWidth, 20);
  ...
  // Convert totals to numbers (using toNumber to handle commas)
  ...
  const totalClosingNum = toNumber(totalClosing);
  ...
  totalRow.eachCell((cell, colNumber) => {
    ...
    if (colNumber > 2) {
      ...
      cell.numFmt = "#,##0"; // no decimal places for totals as well
    }
    if (colNumber === 1) {
      cell.alignment = { horizontal: "center" };
    }
  ...
  saveAs(blob, `BalanceReport As On ${currentdate}.xlsx`);
};
  ...
  return (
    <>
      ...
      <div style={contentStyle}>
        ...
        >
          <NavComponent textdata="Balance Report" />
  ...
  if (selectedOption && selectedOption.value) {
    const labelPart = selectedOption.label.substring(selectedOption.label.lastIndexOf("-") + 1);
    setCompanyselectdata(selectedOption.value);
    ...
    setCompanyselectdatavalue("");
                    ...
                    styles={{
                      ...customStyles1(!Companyselectdata, 400),
                      placeholder: (base) => ({
                        ...base,
                        textAlign: "left",
                        marginLeft: "0",
