...
const exportPDFHandler = () => {
  ...
  const columnWidths = [20, 100, 20, 28, 30];

  // Total table width
  const totalWidth = columnWidths.reduce((acc, w) => acc + w, 0);

  // Page dimensions – maximise rows
  const pageHeight = doc.internal.pageSize.height;
  const paddingTop = 15;
  const footerReserve = 18; // reduced from 30 to pack more rows
  ...
  // ----- Helper: Draw footer line and "Crystal Solution" text (adjusted for tight margin) -----
  const drawFooter = () => {
    const tableWidth = getTotalTableWidth();
    const lineX = (doc.internal.pageSize.width - tableWidth) / 2;
    const lineY = pageHeight - 12; // moved lower to fit in footerReserve
  ...
  // ----- Helper: Draw page number (now with total pages, same Y as footer) -----
  ...
  // ----- Add rows with dynamic pagination (modified to accept totalPages) -----
  const addTableRows = (startX, startY, startIndex, pageNum, totalPages) => {
    ...
    while (currentRowIndex < rows.length) {
      ...
      // Borders (total row double lines)
      if (isTotalRow) {
        doc.setFont("verdana", "bold");
        doc.setLineWidth(0.3);
      ...
      splitRow.forEach((textArray, cellIndex) => {
        ...
        // For total row first cell (index 0) – center it
        const isTotalFirstCell = isTotalRow && cellIndex === 0;
        if (isTotalFirstCell) {
  ...
  // ----- Dry run: compute total pages without drawing (using same tight footerReserve) -----
  const computeTotalPages = () => {
    ...
    let pageCount = 0;
    let nextRowIndex = 0;
    const headersStartY = 39; // same as original calculation
  ...
  // ----- Main pagination handler (now passes totalPages to addTableRows) -----
  const handlePagination = () => {
    ...
    while (nextRowIndex < rows.length) {
      ...
      currentStartY += 5;
      doc.setFont("verdana-regular", "normal");
      addTitle(`Spare Parts Stock Report As on ${toInputDate}`, currentStartY, 12);
      currentStartY += 5;
      ...
      const typename = typeselectdatavalue.label ? typeselectdatavalue.label + "-" + "Stock" : "ALL";
      ...
      doc.setFont("verdana", "bold");
      doc.text(`Rate :`, labelsX + 120, labelsY + 4.3);
      doc.setFont("verdana-regular", "normal");
      doc.text(`${RATE}`, labelsX + 140, labelsY + 4.3);
      ...
      doc.setFont("verdana", "bold");
      doc.text(`Status :`, labelsX + 120, labelsY + 8.5);
      doc.setFont("verdana-regular", "normal");
      doc.text(`${transectionsts}`, labelsX + 140, labelsY + 8.5);

      if (searchQuery) {
        doc.setFont("verdana", "bold");
        doc.text(`Search :`, labelsX + 140, labelsY + 12.5);
        doc.setFont("verdana-regular", "normal");
        doc.text(`${search}`, labelsX + 160, labelsY + 12.5);
      }

      currentStartY += searchQuery ? 25 : 20;

      // === TABLE HEADERS ===
      const headersStartX = (doc.internal.pageSize.width - totalWidth) / 2;
      const headersStartY = searchQuery ? 44 : 39;
      addTableHeaders(headersStartX, headersStartY);

      // === TABLE ROWS (pass totalPages) ===
      const rowsStartY = headersStartY + 6;
...
const handleDownloadCSV = async () => {
  ...
  // Add data rows with numeric conversion for Rate, Qnty, Amount
  tableData.forEach((item) => {
    ...
    row.eachCell((cell, colIndex) => {
      ...
      // Rate (col4) and Qnty (col5) – keep two decimals
      // Amount (col6) – no decimals
  ...
  // Convert totals to numbers (remove commas)
  ...
  totalRow.eachCell((cell, colNumber) => {
    ...
    if (colNumber === 5) {
      ...
      cell.numFmt = "#,##0.00"; // Qnty total with decimals
    } else if (colNumber === 6) {
      cell.alignment = { horizontal: "right" };
      cell.numFmt = "#,##0"; // Amount total without decimals
    }
