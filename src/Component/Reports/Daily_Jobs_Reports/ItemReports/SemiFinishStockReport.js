...
export default function SemiFinishStockReport() {
  ...
  function fetchDailyStatusReport() {
    ...
    const apiMainUrl = apiLinks + "/ItemStockReportSwissCare.php";
    setIsLoading(true);
  ...
  const customStyles1 = (hasError) => ({
   ...
   const exportPDFHandler = () => {
      ...
      const columnWidths = [20, 95,  25, 25, 30];
      ...
      const handlePagination = () => {
        ...
        while (currentPageIndex * rowsPerPage < rows.length) {
          ...
          doc.setFont('verdana-regular', "normal");
           doc.setFontSize(10);
          addTitle(
            `Semi Finish Stock Report at ${toInputDate}`,
            "",
            "",
            pageNumber,
            startY,
         ...
         doc.setFontSize(10); // Font size
          ...
          if (searchQuery) {
            doc.setFont(getfontstyle, "bold"); // Set font to bold
            doc.text(`SEARCH :`, labelsX + 120, labelsY ); // Draw bold label
            doc.setFont(getfontstyle, "normal"); // Reset font to normal
            doc.text(`${search}`, labelsX + 140, labelsY ); // Draw the value next to the label
          }
  
          // // Reset font weight to normal if necessary for subsequent text
          
  
          startY += searchQuery ? 1 : 0; // Adjust vertical position for the labels
  
          addTableHeaders((doc.internal.pageSize.width - totalWidth) / 2, searchQuery ? 30 : 29);
      ...
      // Save the PDF files
      doc.save(`SemiFinishStockReport As On ${date}.pdf`);
    };
 ...
 const typeAndStoreRow4 = worksheet.addRow(
     ...
     worksheet.mergeCells(`D${typeAndStoreRow4.number}:E${typeAndStoreRow4.number}`);

typeAndStoreRow4.eachCell((cell, colIndex) => {
      cell.font = {
        name: "CustomFont" || "CustomFont",
        size: 10,
        bold: [3].includes(colIndex),
      };
      ...
      // Set column widths
      [10,45,14,10, 14].forEach((width, index) => {
      ...
      saveAs(blob, `SemiFinishStockReport As On ${currentdate}.xlsx`);
    };
  ...
  return (
    <>
      ...
      <div style={contentStyle}>
        ...
        >
          <NavComponent textdata="Semi Finish Stock Report" />
