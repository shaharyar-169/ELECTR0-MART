...
export default function RegionList() {
  ...
  function fetchReceivableReport() {
    const apiUrl = apiLinks + "/RegionList.php";
    setIsLoading(true);
    const formData = new URLSearchParams({
      FRegSts: transectionType,
  ...
  const exportPDFHandler = () => {
    ...
    const headers = ["Code", "Region", "Status"];
    ...
    const handlePagination = () => {
      ...
      while (currentPageIndex * rowsPerPage < rows.length) {
        ...
        addTitle(`Region List`, "", "", pageNumber, startY, 12); // Render sale report title with decreased font size, provide the time, and page number
        startY += -5;
    ...
    // Save the PDF files
    doc.save(`RegionList As On ${date}.pdf`);
  };
  ///////////////////////////// DOWNLOAD PDF CODE ////////////////////////////////////////////////////////////
  ...
  const handleDownloadCSV = async () => {
       ...
       // Add Store List row
       const storeListRow = worksheet.addRow(["Region List"]);
       ...
       tableData.forEach((item) => {
         const row = worksheet.addRow([item.Code, item.Region, item.Status]);
       ...
       saveAs(blob, `RegionList As On ${currentdate}.xlsx`);
     };
  ///////////////////////////// DOWNLOAD PDF EXCEL ///////////////////////////////////////////////////////////
  ...
  return (
    <>
      <div style={contentStyle}>
        ...
        >
          <NavComponent textdata="Region List" />
