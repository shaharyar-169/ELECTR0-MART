...
//        doc.setFont("verdana-regular", "normal");
//          addTitle(`Company List`, "", "", pageNumber, startY, 12); // Render sale report title with decreased font size, provide the time, and page number
//          startY += -5;
...
//      // Save the PDF files
//      doc.save(`CompanyList As On ${date}.pdf`);
//    };
//    ///////////////////////////// DOWNLOAD PDF CODE ////////////////////////////////////////////////////////////
...
//      });
//      saveAs(blob, `CompanyList As On ${currentDate}.xlsx`);
//    };
...
//           }}
//         >
//           <NavComponent textdata="Company List" />
...
export default function AmericanSaleManInfo() {
  const navigate = useNavigate();
  const user = getUserData();
  const organisation = getOrganisationData();
  const yeardescription = getYearDescription();
  ...
  const [TotalsSalesman, setTotalsSalesman] = useState([]);
  ...
  function fetchReceivableReport() {
    const apiUrl = apiLinks + "/AmericanSaleManInfo.php";
    setIsLoading(true);
    const formData = new URLSearchParams({
      // FCmpSts: transectionType,
      code: organisation.code,
    ...
    axios
      ...
      .then((response) => {
        ...
        setTotalsSalesman(response.data["Total Balance"]);
        setTotalNos(response.data["Total Customers"]);
        setTotalBal(response.data["Total SalesMan"]);
  ...
  ///////////////////////////// DOWNLOAD PDF CODE ////////////////////////////////////////////////////////////
   const exportPDFHandler = () => {
     ...
     rows.push([
    String(formatValue(tableData.length.toLocaleString())),
    String(formatValue(TotalsSalesman)),
    String(formatValue(TotalNos)),
    String(formatValue(TotalBal)),
    ]);
 
     // Define table column headers and individual column widths
     const headers = ["Code", "Salesman", "Nos", "Balance"];
     const columnWidths = [15, 110, 25,25];
     ...
     const handlePagination = () => {
       ...
       while (currentPageIndex * rowsPerPage < rows.length) {
        ...
        addTitle(comapnyname, 12, 12, pageNumber, startY, 18); // Render company title with default font size, only date, and page number
         startY += 5; // Adjust vertical position for the company title
       doc.setFont("verdana-regular", "normal");
         addTitle(`American SaleMan Info Report`, "", "", pageNumber, startY, 12); // Render sale report title with decreased font size, provide the time, and page number
         startY += -5;
  ...
  //        }
     ...
     // Save the PDF files
     doc.save(`AmericanSalemaninfoReport As On ${date}.pdf`);
   };
   ///////////////////////////// DOWNLOAD PDF CODE ////////////////////////////////////////////////////////////
   ...
   const handleDownloadCSV = async () => {
     ...
     worksheet.getRow(companyRow.number).height = 30;
     worksheet.mergeCells(`A${companyRow.number}:${String.fromCharCode(66 + numColumns - 1)}${companyRow.number}`);
 
     // Store List
     const storeListRow = worksheet.addRow(["American SaleMan Info Report"]);
     storeListRow.eachCell((cell) => {
       cell.font = fontStoreList;
       cell.alignment = { horizontal: "center" };
     });
     worksheet.mergeCells(`A${storeListRow.number}:${String.fromCharCode(66 + numColumns - 1)}${storeListRow.number}`);
    ...
    //  });
     ...
     // Column widths
     [7, 45,12,12].forEach((width, index) => {
       worksheet.getColumn(index + 1).width = width;
     });
    ...
    String(formatValue(TotalBal)),
     ...
     saveAs(blob, `AmericanSaleManInfoReport  As On ${currentDate}.xlsx`);
   };
  ...
  useEffect(() => {
    if (tableData.length > 0) {
      const newColumns = {
        tsalcod: tableData.map((row) => row.tsalcod),
        SalesMan: tableData.map((row) => row.SalesMan),
        Nos: tableData.map((row) => row.Nos),
                Bal: tableData.map((row) => row.Bal),
  ...
  const firstColWidth = { width: "50px" };
  const secondColWidth = { width: "360px" };
  ...
  return (
    <>
      <div style={contentStyle}>
        ...
        >
          <NavComponent textdata="American Saleman Info Report" />
