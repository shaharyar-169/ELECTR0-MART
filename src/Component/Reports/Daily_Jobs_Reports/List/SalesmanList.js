...
export default function SalesmanList() {
  ...
  function fetchReceivableReport() {
    const apiUrl = apiLinks + "/SalesmanList.php";
    setIsLoading(true);
    const formData = new URLSearchParams({
    FSalSts: transectionType,
...
const exportPDFHandler = () => {
  ...
  for (let i = startIndex; i < endIndex; i++) {
    ...
    const rowTop = startY + (i - startIndex + 2) * rowHeight;
    const rowBottom = rowTop + rowHeight;

    // ✅ Normal outer rectangle for every row (keeps grid consistent)
    doc.setLineWidth(0.2);
    doc.rect(startX, rowTop, tableWidth, rowHeight);

    // ✅ TRUE DOUBLE TOP & BOTTOM border for total row
    if (isTotalRow) {
      // Top border (double)
      doc.setLineWidth(0.2);
      doc.line(startX, rowTop, startX + tableWidth, rowTop);
      doc.line(startX, rowTop + 0.8, startX + tableWidth, rowTop + 0.8);

      // Bottom border (double)
      doc.line(startX, rowBottom - 0.8, startX + tableWidth, rowBottom - 0.8);
      doc.line(startX, rowBottom, startX + tableWidth, rowBottom);
    }

    row.forEach((cell, cellIndex) => {
      // ✅ PERFECT VERTICAL CENTER
      const cellY = rowTop + rowHeight / 2;
  ...
  doc.setFontSize(11);
  doc.setTextColor(0);
  // ✅ No bold on footer text
  doc.setFont("verdana-regular", "normal");
  // ✅ Crystal Solution + Date + Time (values only, no labels)
  doc.text(
    ...
    lineY + 5
     ...
     const handlePagination = () => {
       // ✅ Total pages precomputed for page numbering
 ...
 doc.setFont("verdana-regular", "normal");
       ...
       doc.setFont("verdana-regular", "normal");
         addTitle(`SaleMan List`, "", "", pageNumber, startY, 12); // Render sale report title with decreased font size, provide the time, and page number
         startY += -5;
  ...
  doc.setFont("verdana-regular", "normal");
     ...
     // Save the PDF files
     doc.save(`SaleManList As On ${date}.pdf`);
   };
   ///////////////////////////// DOWNLOAD PDF CODE ////////////////////////////////////////////////////////////
  ...
  const handleDownloadCSV = async () => {
     ...
     // ✅ Set the 3 table column widths FIRST so we know their total width
     const tableColumnWidths = [10, 45, 7]; // same widths as table (unchanged)
     tableColumnWidths.forEach((w, i) => {
     ...
     companyRow.eachCell((cell) => {
       ...
       // ✅ LEFT aligned company name (was center before)
       cell.alignment = { horizontal: "left", vertical: "middle", wrapText: false, shrinkToFit: false };
     });
     worksheet.getRow(companyRow.number).height = 30;
 
     // ✅ Compute how many extra columns are needed so the merged region is wide enough
     const companyText = String(comapnyname ?? "");
     const approxCharsNeeded = Math.ceil(companyText.length * 1.7) + 6;
 
     const tableTotalWidth = tableColumnWidths.reduce((a, b) => a + b, 0);
 
     let mergeEndCol = numColumns;
     let availableWidth = tableTotalWidth;
 
     if (approxCharsNeeded > availableWidth) {
       let stillNeeded = approxCharsNeeded - availableWidth;
       const extraColWidth = 30;
       while (stillNeeded > 0) {
         mergeEndCol += 1;
         worksheet.getColumn(mergeEndCol).width = extraColWidth;
         availableWidth += extraColWidth;
         stillNeeded -= extraColWidth;
       }
     }
 
     // ✅ Merge company row across A..mergeEndCol so the merged cell has enough width
     worksheet.mergeCells(
       `A${companyRow.number}:${String.fromCharCode(65 + mergeEndCol - 1)}${companyRow.number}`
     );
 
     // Store List
     const storeListRow = worksheet.addRow(["SaleMan List"]);
     ...
     // Column widths (idempotent — already set above, kept here for consistency)
     [10, 45, 7].forEach((width, index) => {
     ...
     saveAs(blob, `SaleManList As On ${currentDate}.xlsx`);
   };
  ...
  return (
    <>
      <div style={contentStyle}>
        ...
        >
          <NavComponent textdata="SalesMan List" />
