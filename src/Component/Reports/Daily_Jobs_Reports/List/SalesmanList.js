...
export default function SalesmanList() {
  ...
  const exportPDFHandler = () => {
    ...
    const headers = ["Code", "Description", "Status", "Manager"];
    const columnWidths = [22, 70, 15, 45];
    ...
    const handlePagination = () => {
      ...
      while (currentPageIndex * rowsPerPage < rows.length) {
        ...
        addTitle(`SalesMan List`, "", "", pageNumber, startY, 12); // Render sale report title with decreased font size, provide the time, and page number
        startY += -5;
    ...
    // Save the PDF files
    doc.save(`SalesmanList As On ${date}.pdf`);
  };
  ///////////////////////////// DOWNLOAD PDF CODE ////////////////////////////////////////////////////////////
  ...
  const handleDownloadCSV = async () => {
       ...
       const columnAlignments = ["left", "left", "center","left"];
       ...
       // Add Store List row
       const storeListRow = worksheet.addRow(["SalesMan List"]);
       ...
       tableData.forEach((item) => {
         const row = worksheet.addRow([item.Code, item.Description, item.Status, item.Manager]);
       ...
       // Set column widths
       [10, 40, 10, 25].forEach((width, index) => {
       ...
       saveAs(blob, `SalesManList As On ${currentdate}.xlsx`);
     };
  ///////////////////////////// DOWNLOAD PDF EXCEL ///////////////////////////////////////////////////////////
