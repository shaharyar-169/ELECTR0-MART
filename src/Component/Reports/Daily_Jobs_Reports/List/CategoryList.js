    ...
    const exportPDFHandler = () => {
      ...
      const handlePagination = () => {
        ...
        while (currentPageIndex * rowsPerPage < rows.length) {
          ...
          // startY += 7;
          addTitle(`Category List`, "", "", pageNumber, startY, 14); // Render sale report title with decreased font size, provide the time, and page number
          startY += 13;
    ...
    const handleDownloadCSV = async () => {
      ...
      [comapnyname, `Category List`].forEach((title, index) => {
        ...
        worksheet.mergeCells(
          `A${index + 2}:${String.fromCharCode(60 + numColumns)}${index + 2}`
        );
      });
