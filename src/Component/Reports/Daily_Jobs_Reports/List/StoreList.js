...
//   const [isAscendingabb, setisAscendingabb] = useState(true);
//   const [isAscendingstk, setisAscendingstk] = useState(true);
...
//     const columnWidths = [14, 80, 18, 30, 15];
...
// doc.setFont("verdana-regular", "normal");
//         addTitle(`Store List`, "", "", pageNumber, startY, 12); // Render sale report title with decreased font size, provide the time, and page number
//         startY += -5;
...
//     const columnAlignments = ["center", "left", "center", "left", "center"];
...
//     // Set column widths
//     [7, 30, 8, 11, 5].forEach((width, index) => {
...
//         Abb: tableData.map((row) => row.Abb), // New column
//         Stk: tableData.map((row) => row.Stk), // New column
//       };
...
//                   fontFamily: getfontstyle,
//                   // width: "100%",
//                   tableLayout: "fixed",   // 🔥 Fixes pixel width behavior
//                   position: "relative",
//                 }}
//               >
//                 <thead
...
//                     <td style={secondColWidth} onClick={() => handleSorting("Description")}>
//                       Description <i className="fa-solid fa-caret-down"></i>
//                     </td>

//                     <td style={thirdColWidth} onClick={() => handleSorting("Status")}>
//                       Status <i className="fa-solid fa-caret-down"></i>
//                     </td>

//                     <td style={forthColWidth} onClick={() => handleSorting("Abb")}>
//                       Abb <i className="fa-solid fa-caret-down"></i>
//                     </td>

//                     <td style={fifthColWidth} onClick={() => handleSorting("Stk")}>
//                       Stk <i className="fa-solid fa-caret-down"></i>
//                     </td>

//                     <td style={sixthcol} onClick={() => handleSorting("Stk")}>
//                     </td>
...
export default function LocationList() {
  ...
  function fetchReceivableReport() {
    const apiUrl = apiLinks + "/GetLocations.php";
    setIsLoading(true);
    const formData = new URLSearchParams({
      code: organisation.code,
      FLocCod: locationnumber || getLocationNumber,
  ...
  ///////////////////////////// DOWNLOAD PDF CODE ////////////////////////////////////////////////////////////
   const exportPDFHandler = () => {
     ...
     // Define table column headers and individual column widths
     const headers = ["Code", "Name", "Address", 'Sts'];
     const columnWidths = [15, 110,110, 15];
 ...
 doc.setFont("verdana-regular", "normal");
       ...
       while (currentPageIndex * rowsPerPage < rows.length) {
        ...
        addTitle(comapnyname, 12, 12, pageNumber, startY, 18); // Render company title with default font size, only date, and page number
         startY += 5; // Adjust vertical position for the company title
       doc.setFont("verdana-regular", "normal");
         addTitle(`Location List`, "", "", pageNumber, startY, 12); // Render sale report title with decreased font size, provide the time, and page number
         startY += -5;
...
//          doc.text(`${status}`, labelsX + 20, labelsY + 8.5); // Draw the value next to the label
  ...
  doc.setFont("verdana", "bold");
     doc.setFontSize(10);
               doc.text(`Search :`, labelsX , labelsY + 8.5); // Draw bold label
  doc.setFont("verdana-regular", "normal");
     doc.setFontSize(10);
     ...
     // Save the PDF files
     doc.save(`LocationList As On ${date}.pdf`);
   };
   ///////////////////////////// DOWNLOAD PDF CODE ////////////////////////////////////////////////////////////
   ...
   const handleDownloadCSV = async () => {
     ...
     const columnAlignments = ["center", "left", "left","center"];
     ...
     // Store List
     const storeListRow = worksheet.addRow(["Location List"]);
     storeListRow.eachCell((cell) => {
       cell.font = fontStoreList;
       cell.alignment = { horizontal: "center" };
     });
 ...
 item.tloccod,
     ...
     saveAs(blob, `LocationList As On ${currentDate}.xlsx`);
   };
 ...
 tloccod: "",
  ...
  useEffect(() => {
    if (tableData.length > 0) {
      const newColumns = {
        tloccod: tableData.map((row) => row.tloccod),
        tlocnam: tableData.map((row) => row.tlocnam),
        tlocadd: tableData.map((row) => row.tlocadd),
        tlocsts: tableData.map((row) => row.tlocsts),
  ...
  return (
    item.tloccod?.toLowerCase().includes(query) ||
    item.tlocnam?.toLowerCase().includes(query) ||
    item.tlocadd?.toLowerCase().includes(query) ||
    item.tlocsts?.toLowerCase().includes(query)
  );
});
  ...
  return (
    <>
      <div style={contentStyle}>
        ...
        >
          <NavComponent textdata="Location List" />
                ...
                >
                  ...
                  >
                    <td
                      className="border-dark"
                      style={firstColWidth}
                      onClick={() => handleSorting("tloccod")}
                    >
                      Code{" "}
                      <i
                        className="fa-solid fa-caret-down caretIconStyle"
                        style={getIconStyle("tloccod")}
                      ></i>
                    </td>
                    ...
                    <td
                      className="border-dark"
                      style={secondColWidth}
                      onClick={() => handleSorting("tlocnam")}
                    >
                      Name{" "}
                      <i
                        className="fa-solid fa-caret-down caretIconStyle"
                        style={getIconStyle("tlocnam")}
                      ></i>
                    </td>
                    ...
                    <td
                      className="border-dark"
                      style={thirdColWidth}
                      onClick={() => handleSorting("tlocadd")}
                    >
                      Address{" "}
                      <i
                        className="fa-solid fa-caret-down caretIconStyle"
                        style={getIconStyle("tlocadd")}
                      ></i>
                    </td>
                      ...
                      className="border-dark"
                      style={forthColWidth}
                      onClick={() => handleSorting("tlocsts")}
                    >
                      Sts{" "}
                      <i
                        className="fa-solid fa-caret-down caretIconStyle"
                        style={getIconStyle("tlocsts")}
                      ></i>
                    </td>
