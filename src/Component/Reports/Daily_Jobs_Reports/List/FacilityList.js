...
export default function FacilityList() {
    ...
    function fetchReceivableReport() {
        const apiUrl = apiLinks + "/FacilityList.php";
        setIsLoading(true);
    ...
    const exportPDFHandler = () => {
        ...
        const handlePagination = () => {
            ...
            while (currentPageIndex * rowsPerPage < rows.length) {
                ...
                addTitle(`Facility List`, "", "", pageNumber, startY, 12); // Render sale report title with decreased font size, provide the time, and page number
                startY += -5;
        ...
        // Save the PDF files
        doc.save(`FacilityList As On ${date}.pdf`);
    };
    ///////////////////////////// DOWNLOAD PDF CODE ////////////////////////////////////////////////////////////
    ...
    const handleDownloadCSV = async () => {
        ...
        const columnAlignments = ["left", "left", "center", "left", "left", "left", "left"];
        ...
        worksheet.mergeCells(`A${companyRow.number}:${String.fromCharCode(69 + numColumns - 1)}${companyRow.number}`);
        ...
        worksheet.mergeCells(`A${storeListRow.number}:${String.fromCharCode(69 + numColumns - 1)}${storeListRow.number}`);
        ...
        // Add headers
        const headers = ["Code", "Description", "Status", 'Ins ID', "Ins date", "Upd ID", "Upd Date"];
        ...
        // Set column widths
        [10, 30, 10,30, 12,12,12].forEach((width, index) => {
        ...
        saveAs(blob, `SlotList As On ${currentdate}.xlsx`);
    };
    ...
    useEffect(() => {
        if (tableData.length > 0) {
            const newColumns = {
                ...
                "Ins ID": tableData.map(row => row["Ins ID"]),
                "Ins Date": tableData.map(row => row["Ins Date"]),
                "Upd ID": tableData.map(row => row["Upd ID"]),
                "Upd Date": tableData.map(row => row["Upd Date"])
            };
    ...
    const renderTableData = () => {
        const rowCount = Math.max(
            ...
            columns["Ins ID"].length,
            columns["Ins Date"].length,
            columns["Upd ID"].length,
            columns["Upd Date"].length
        );
        ...
        for (let i = 0; i < rowCount; i++) {
            rows.push({
                ...
                "Ins ID": columns["Ins ID"][i],
                "Ins Date": columns["Ins Date"][i],
                "Upd ID": columns["Upd ID"][i],
                "Upd Date": columns["Upd Date"][i]
            });
        }
    ...
    return (
        <>
            <div style={contentStyle}>
                ...
                >
                    <NavComponent textdata="Facility List" />
