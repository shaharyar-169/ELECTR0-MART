...
export default function CategoryListPos() {
  ...
  const [isAscendingptccode, setisAscendingptccode] = useState(true);
  ...
  const exportPDFHandler = () => {
    ...
    const headers = ["Code", "Description", 'PTC Code', "Status"];
    const columnWidths = [22, 90, 22, 15];
    ...
    const addTableRows = (startX, startY, startIndex, endIndex) => {
      ...
      for (let i = startIndex; i < endIndex; i++) {
        ...
        row.forEach((cell, cellIndex) => {
          ...
          if (cellIndex === 3 || cellIndex === 4 || cellIndex === 6) {
    ...
    const handlePagination = () => {
      ...
      while (currentPageIndex * rowsPerPage < rows.length) {
        ...
        addTitle(`Category List`, "", "", pageNumber, startY, 12); // Render sale report title with decreased font size, provide the time, and page number
        startY += -5;
    ...
    // Save the PDF files
    doc.save(`CategoryList As On ${date}.pdf`);
  };
  ///////////////////////////// DOWNLOAD PDF CODE ////////////////////////////////////////////////////////////
  ...
  const handleDownloadCSV = async () => {
    ...
    tableData.forEach((item) => {
      const row = worksheet.addRow([item.Code, item.Description, item['PCT Code'], item.Status]);
    ...
    // Set column widths
    [10, 40, 12, 10].forEach((width, index) => {
    ...
    saveAs(blob, `CategoryList As On ${currentdate}.xlsx`);
  };
  ///////////////////////////// DOWNLOAD PDF EXCEL ///////////////////////////////////////////////////////////
  ...
  const handleSorting = async (col) => {
    ...
    } else if (col === "PCT Code") {
      setisAscendingptccode(newSortOrder === "ASC");
    }
    else if (col === "Status") {
                ...
                >
                  ...
                  >
                    ...
                    <td
                      className="border-dark"
                      style={forthColWidth}
                      onClick={() => handleSorting("PCT Code")}
                    >
                      PCT Code{" "}
                      <i className="fa-solid fa-caret-down caretIconStyle"
                        style={{
                          transform: isAscendingptccode ? "rotate(0deg)" : "rotate(180deg)", // 180deg for better visual
                          color: isAscendingptccode ? "white" : "red",
                          transition: "transform 0.3s ease",
                        }}
                      ></i>
                    </td>
