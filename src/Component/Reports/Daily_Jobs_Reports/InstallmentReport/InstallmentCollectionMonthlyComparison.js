...
import { Container, Modal, Spinner, Nav } from "react-bootstrap";
...
export default function InstallmentCollectionMonthlyComparison() {
  ...
  const [SaleCust, setSaleCust] = useState(0);
  ...
  const [selectedHeaderField, setSelectedHeaderField] = useState(null);
  ...
  function fetchReceivableReport() {
    ...
    const apiUrl = apiLinks + "/InstallmentCollectionMonthlyComparison.php";
    setIsLoading(true);
    ...
    axios
      ...
      .then((response) => {
        ...
        setSaleCust(response.data["SaleCust"]);
        ...
        setTotalCust(response.data["TotalCust"]);
        setTotalIns(response.data["TotalIns"]);
        setTotalCol(response.data["TotalCol"]);
        setTotalDif(response.data["TotalDif"]);
  ...
  const exportPDFHandler = () => {
    ...
    const columnWidths = [12, 24, 12, 27, 27, 18, 27, 27, 18, 15, 27, 27, 27];
    ...
    const handlePagination = () => {
      ...
      while (currentPageIndex * rowsPerPage < rows.length) {
        ...
        doc.setFont("verdana-regular", "normal");
        addTitle(
          `Installment Collection Monthly Comparison From ${fromInputDate} To ${toInputDate}`,
          "",
          "",
          pageNumber,
          startY,
        ...
        doc.setFont("verdana", "bold");
        doc.setFontSize(10);
        doc.text(`Types :`, labelsX + 180, labelsY + 8.5); // Draw bold label
        doc.setFont("verdana-regular", "normal");
        doc.setFontSize(10);
        doc.text(`${status}`, labelsX + 200, labelsY + 8.5); // Draw the value next to the label
    ...
    // Save the PDF files
    doc.save(
      `InastallmentcollectMonthlyComparison from ${fromInputDate} to ${toInputDate}.pdf`,
    );
  };
  ///////////////////////////// DOWNLOAD PDF CODE ////////////////////////////////////////////////////////////
  ///////////////////////////// DOWNLOAD PDF EXCEL //////////////////////////////////////////////////////////
  const handleDownloadCSV = async () => {
    ...
    worksheet.mergeCells(
      `C${typeAndStoreRow.number}:E${typeAndStoreRow.number}`,
    );

    // Columns that should be RIGHT aligned (values)
    const rightAlignCols = [9];
    ...
    // Set column widths
    [6, 12, 8, 14, 14, 14, 14, 14, 14, 14, 14, 14, 14].forEach(
      (width, index) => {
    ...
    saveAs(
      blob,
      `InstallmentCollectionMonthlyComparison From ${fromInputDate} To ${toInputDate}.xlsx`,
    );
  };
  ///////////////////////////// DOWNLOAD PDF EXCEL ///////////////////////////////////////////////////////////
  ...
  const secondColWidth2 = {
    width: isSidebarVisible ? "40px" : "40px",
  };
  const secondColWidth3 = {
    width: isSidebarVisible ? "83px" : "90px",
  };
  const thirdColWidth = {
    width: isSidebarVisible ? "83px" : "90px",
  };
  ...
  const handleHeaderChart = (fieldName) => {
    if (!tableData?.length) return;

    const chartData = tableData.map((row) => ({
      Month: row.Month,
      value: Number((row[fieldName] ?? 0).toString().replace(/,/g, "")) || 0,
    }));

    setSelectedHeaderField(fieldName); // 👈 important
    setSelectedHeaderData(chartData);
    setShow(true);
  };

  const headerTotalsMap = {
    SaleCust: undefined,
    SaleAmt: TotalSale,
    RentAmt: TotalRent,
    RentAge: undefined,
    TotalAmt: TotalAmount,
    AdvaceAmt: TotalAdvance,
    AdvanceAge: undefined,
    CustNos: TotalCust,
    InsAmt: TotalIns,
    Collection: TotalCol,
    Diff: TotalDif,
  ...
  const monthLabels =
    selectedHeaderData?.map((d) => d.Month?.slice(0, 3)) || [];
    ...
    datasets: [
      {
        ...
        data: selectedHeaderData?.map((d) => d.value) || [],
  ...
  let HeaderLables =
    selectedHeaderField === "SaleCust"
      ? "Sale Cust"
      : selectedHeaderField === "SaleAmt"
        ? "Sale Amount"
        : selectedHeaderField === "RentAmt"
          ? "Rent Amount"
          : selectedHeaderField === "RentAge"
            ? "Rent Percentage"
            : selectedHeaderField === "TotalAmt"
              ? "Total Amount"
              : selectedHeaderField === "AdvaceAmt"
                ? "Advance Amount"
                : selectedHeaderField === "AdvanceAge"
                  ? "Advance Percentage"
                  : selectedHeaderField === "CustNos"
                    ? "Customer"
                    : selectedHeaderField === "InsAmt"
                      ? "Installment Amount"
                      : selectedHeaderField === "Collection"
                        ? "Collection"
                        : selectedHeaderField === "Diff"
                          ? "Difference"
                          : "";
  ...
  return (
    <>
      <Modal show={show} onHide={() => setShow(false)} centered>
        <Modal.Header closeButton>
          <Modal.Title style={{ fontSize: "16px" }}>
            Installment Collection Monthly Comparison
          </Modal.Title>
        </Modal.Header>
        ...
        >
          ...
          >
            <strong>{HeaderLables}</strong>
            {headerTotalsMap[selectedHeaderField] != null &&
              headerTotalsMap[selectedHeaderField] !== "" && (
                <>
                  <br />
                  <strong>Total :</strong>{" "}
                  {headerTotalsMap[selectedHeaderField].toLocaleString()}
                </>
              )}
          </p>
      ...
      <div style={contentStyle}>
        ...
        >
          <NavComponent textdata="Installment Collectoin Monthly Comparison" />
          <div
            className="row"
                ...
                >
                  ...
                  >
                    ...
                    <td
                      ...
                      onClick={() => handleHeaderChart("SaleCust")}
                      style={{ ...secondColWidth2, cursor: "pointer" }}
                    >
                      Cst#
                    </td>
                    ...
                    <td
                      className="border-dark"
                      onClick={() => handleHeaderChart("SaleAmt")}
                    ...
                    <td
                      className="border-dark"
                      onClick={() => handleHeaderChart("RentAmt")}
                    ...
                    <td
                      className="border-dark"
                      onClick={() => handleHeaderChart("RentAge")}
                      style={{ ...ninhthColWidth, cursor: "pointer" }}
                    >
                      Rent %
                    </td>
                    ...
                    <td
                      className="border-dark"
                      onClick={() => handleHeaderChart("TotalAmt")}
                    ...
                    <td
                      className="border-dark"
                      onClick={() => handleHeaderChart("AdvanceAmt")}
                    ...
                    <td
                      className="border-dark"
                      onClick={() => handleHeaderChart("AdvanceAge")}
                      style={{ ...ninhthColWidth, cursor: "pointer" }}
                    >
                      Adv %
                    </td>
                    <td
                      className="border-dark"
                      onClick={() => handleHeaderChart("CustNos")}
                    ...
                    <td
                      className="border-dark"
                      onClick={() => handleHeaderChart("InsAmt")}
                    ...
                    <td
                      className="border-dark"
                      onClick={() => handleHeaderChart("Collection")}
                      style={{ ...ninhthColWidth, cursor: "pointer" }}
                    >
                      Collection
                    </td>
                    <td
                      className="border-dark"
                      onClick={() => handleHeaderChart("Diff")}
                      style={{ ...ninhthColWidth, cursor: "pointer" }}
                    >
                      Diff
                    </td>
                ...
                <tbody id="tablebody">
                  {isLoading ? (
                    <>
                      ...
                      {Array.from({ length: Math.max(0, 15 - 5) }).map(
                        (_, rowIndex) => (
                          <tr
                            key={`blank-${rowIndex}`}
                            style={{
                      ...
                      <tr>
                        ...
                        <td style={secondColWidth2}></td>
                        {/* <td style={secondColWidth3}></td> */}
                    ...
                    <>
                      {tableData.map((item, i) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <td className="text-end" style={secondColWidth2}>
                              {item.SaleCust}
                            </td>
                      ...
                      {Array.from({
                        length: Math.max(0, 13 - tableData.length),
                      }).map((_, rowIndex) => (
                        <tr
                          key={`blank-${rowIndex}`}
                          style={{
