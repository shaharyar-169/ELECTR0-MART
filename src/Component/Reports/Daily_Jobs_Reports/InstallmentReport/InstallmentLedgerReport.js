...
//     // Profession / officeContact /PrmDatet
...
const exportPDFHandler = () => {
  ...
  const drawFormHeader = (doc, startY) => {
    ...
    drawField("Advance",     (
                         parseFloat(
                          headerData?.Advance?.replace(/,/g, "") || 0,
                        ) -
                        parseFloat(headerData?.Advance?.replace(/,/g, "") || 0)
                      ).toLocaleString(), rightX, y, 50, true);
    y += fieldHeight + gapY;

    drawField("Collector",  headerData.Collector, leftX, y, 90);
    drawField("Ins Amt", headerData.InsAmt, rightX, y, 50, true);
    y += fieldHeight + gapY;
    ...
    // Profession / officeContact / PrmDate & ExpiryDate in same row
    drawField("Profession", headerData.Profession, leftX, y, 90);
    drawField("officeContact", headerData.OfficeContact, rightX - 30, y, 30);
    drawField("PrmDate", headerData.PrmDate, thirdColX-30, y, 25, true);
    
    // ExpiryDate added in the same row, next to PrmDate
    drawField("ExpiryDate", headerData.ExpiryDate, thirdColX+25, y, 30, true);
    y += fieldHeight + gapY;
...
{headerData.NicNum}                  
              ...
              >
                ...
                >
                  ...
                  <div
  style={{
    color: new Date(headerData.ExpiryDate) < new Date() ? "red" : fontcolor,
    fontSize: "12px",
    height: "100%",
    backgroundColor: getcolor,
    width: "80px",
    ...
    alignItems: "center",
    paddingRight: "5px",
    fontWeight: new Date(headerData.ExpiryDate) < new Date() ? "bold" : "normal",
  }}
>
