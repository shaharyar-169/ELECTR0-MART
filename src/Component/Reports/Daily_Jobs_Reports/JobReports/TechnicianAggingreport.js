...
export default function TechnicianAggingReport() {
  ...
  const [transectionType2, settransectionType2] = useState("D");
  ...
  const exportPDFHandler = () => {
    ...
    rows.push([
      "",
      "Total",
      String(formatValue(totaltwo.toLocaleString())),
      String(formatValue(totalfive.toLocaleString())),
      String(formatValue(totalseven.toLocaleString())),
      String(formatValue(totalten.toLocaleString())),
      String(formatValue(totaltenplus.toLocaleString())),
      String(formatValue(totalQnty.toLocaleString())),
    ]);
    ...
    const handlePagination = () => {
      ...
      while (currentPageIndex * rowsPerPage < rows.length) {
        ...
        addTitle(
          `Technician Agging Report From ${fromInputDate} To ${toInputDate}`,
          "",
          "",
          pageNumber,
          startY,
        ...
        doc.setFont(getfontstyle, "bold"); // Set font to bold
        doc.text(`TYPE :`, labelsX, labelsY); // Draw bold label
        doc.setFont(getfontstyle, "normal"); // Reset font to normal
        doc.text(`${transectionsts}`, labelsX + 15, labelsY); // Draw the value next to the label
    ...
    // Save the PDF files
    doc.save(`TechnicianAggingReport As On ${date}.pdf`);
  };
  ...
  const handleDownloadCSV = async () => {
    ...
    // Set column widths
    [6, 40, 10, 10, 10, 10, 10, 10].forEach((width, index) => {
    ...
    saveAs(blob, `TechnicianAggingReport As On ${currentdate}.xlsx`);
  };
  ...
  useEffect(() => {
    if (tableData.length > 0) {
      const newColumns = {
        ...
        Technician: tableData.map((row) => row.Technician),
        Two: tableData.map((row) => row.Two),
        Five: tableData.map((row) => row.Five),
        Seven: tableData.map((row) => row.Seven),
        Ten: tableData.map((row) => row.Ten),
        TenPlus: tableData.map((row) => row.TenPlus),
              ...
              >
                ...
                >
                  <input
                    ...
                    onKeyDown={(e) => handleToKeyPress(e, input4Ref)}
                    id="toDatePicker"
                    autoComplete="off"
                    placeholder="dd-mm-yyyy"
