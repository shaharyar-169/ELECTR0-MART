...
import "./Complain_Maitenance.css";
import Footer from "../../MainComponent/Footer/Footer";
import { Modal } from "react-bootstrap";
import { fetchData } from "../../react_query/React_Query_Function";
import Complain_Maintenance_Modal from "./Complain_Maintenance_Modal";
import NavComponent from "../../MainComponent/Navform/navbarform";
import ButtonGroupp from "../../MainComponent/Button/ButtonGroup/ButtonGroup";
import { useQuery, useQueryClient } from "@tanstack/react-query";
import {
  fetchNewComplainData,
  fetchComplain,
} from "./Complain_Maintenance_Api";

function Complain_Maintenance() {
  ...
  const {
    data: newComplain,
    error: newComplainError,
    isLoading: isLoadingNewComplain,
  } = useQuery({
    queryKey: ["newComplain"],
    queryFn: fetchNewComplainData,
  });
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      ...
      formDataa.append("FCptCod", formData.AccountCodeform);
      formDataa.append("FCptDsc", formData.Descriptionform);
      formDataa.append("FCptSts", formData.Status);
      ...
      const response = await axios.post(
        `https://crystalsolutions.com.pk/complaint/SaveComplaint.php`,
        formDataa,
        {
          headers: {
  ...
  const [textdata, settextdata] = useState(" Complain Maintenance");

  const handleCloseModal = () => {
    setData({ columns: [], rows: [] });
    setSearchText("");
    setHighlightedRowIndex(0);
    settextdata("Update Complain Maintenance");
  ...
  const fetchDataAndDisplayy = async () => {
    ...
    if (Array.isArray(complain)) {
      setDataa({ columns, rows: complain });
    } else {
      console.warn(
  ...
  const handleBlurRVC = (e) => {
    ...
    setTimeout(() => {
      const selectedItem = dataa.rows.find((item) => item.tcptcod === value);
      ...
      if (selectedItem) {
        setFormData({
          ...formData,
          AccountCodeform: selectedItem.tcptcod,
          Descriptionform: selectedItem.tcptdsc,
          Status: selectedItem.tcptsts,
  ...
  const handleInputChange = (e) => {
    ...
    if (name === "AccountCodeform") {
      ...
      const selectedItem = dataa.rows.find(
        (item) => item.tcptcod === formatToThreeDigits(formattedValue)
      );
  ...
  const filteredRows =
    ...
    data.rows.filter(
      (row) =>
        (row.tcptcod &&
          row.tcptcod.toLowerCase().includes(searchText.toLowerCase())) ||
        (row.tcptdsc &&
          row.tcptdsc.toLowerCase().includes(searchText.toLowerCase()))
    );
        ...
        >
          ...
          >
            <div className="col-md-12 form-item-container">
                      ...
                      <Form.Control
                        ...
                        onKeyDown={(e) => {
                          if (e.key === "Enter") {
                            ...
                            const selectedItem = dataa.rows.find(
                              (item) => item.tcptcod === upperCaseValue
                            );
              ...
              <Complain_Maintenance_Modal
                isOpen={isModalOpen}
                handleClose={handleCloseModal}
                title="Select Complain"
...
export default Complain_Maintenance;
