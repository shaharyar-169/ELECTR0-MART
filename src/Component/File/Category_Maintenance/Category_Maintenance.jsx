...
import GeneralTwoFieldsModal from "./Category_Maintenance_Modal";
import { newCompanyData, getcompanyData } from "./Category_Maintenance_Api";
...
function Category_Maintenance() {
  ...
  //   if (selectedImage1) {
  //     const imgElement = document.getElementById("pic-preview");
  //     imgElement.src = URL.createObjectURL(selectedImage1);
  //   }
  // }, [selectedImage1]);
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      ...
      formDataa.append("FCtgCod", formData.AccountCodeform);
      formDataa.append("FCtgDsc", formData.Descriptionform);
      formDataa.append("FCtgSts", formData.Status);
      formDataa.append("FUrdDsc", geturdu);
      formDataa.append("FWebInd", formData.inputform4);
      formDataa.append("FCtgRem", formData.inputform5);
      formDataa.append("pic", selectedImage1);
      formDataa.append("FPrjId", user?.tprjid);

      console.log("Submitting Form Data:", formDataa);

      const response = await axios.post(
        `https://crystalsolutions.com.pk/csorder3/files/categorymaintenance/SaveCatg.php`,
        formDataa,
        {
          headers: {
  ...
  const [textdata, settextdata] = useState(" Category Maintenance");

  const handleCloseModal = () => {
    setData({ columns: [], rows: [] });
    setSearchText("");
    setHighlightedRowIndex(0);
    settextdata("Update Category Maintenance");
    setModalOpen(false);
  };
  ...
  const handleBlurRVC = (e) => {
    ...
    setTimeout(() => {
      const selectedItem = dataa.find((item) => item.tctgcod === value);
      ...
      if (selectedItem) {
        setFormData({
          ...formData,
          AccountCodeform: selectedItem.tctgcod,
          Descriptionform: selectedItem.tctgdsc,
          Status: selectedItem.tctgsts,
          inputform4: selectedItem.twebind,
          inputform5: selectedItem.tctgrem,
        ...
        const imgElement = document.getElementById("pic-preview");
        imgElement.src = selectedItem.tctgpic;
        setSelectedImage1(selectedItem.tctgpic);

        handlePrediction(selectedItem.tctgdsc).then((result) => {
          setGeturdu(result);
        });
        console.log(
          "show the image address",
          imageurl + removeParentDirectories(selectedImage1)
        );
      } else {
        setFormData({
          ...formData,
                          ...
                          onKeyDown={(e) => {
                            if (e.key === "Enter") {
                              ...
                              if (dataa && dataa.length > 0) {
                                const selectedItem = dataa.find(
                                  (item) => item.tctgcod === upperCaseValue
                                );
...
export default Category_Maintenance;
