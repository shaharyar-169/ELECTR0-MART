...
function Type_Maintenance() {
  ...
  const newcode = async () => {
    try {
      const response = await fetch(
        `https://crystalsolutions.com.pk/umair_electronic/web/NewType.php`
      );
      if (!response.ok) {
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      ...
      formDataa.append("FTypCod", formData.AccountCodeform);
      formDataa.append("FTypDsc", formData.Descriptionform);
      formDataa.append("FTypSts", formData.Status);
      ...
      const response = await axios.post(
        `https://crystalsolutions.com.pk/umair_electronic/web/SaveType.php`,
        formDataa,
        {
          headers: {
  ...
  const [textdata, settextdata] = useState(" Type Maintenance");

  const handleCloseModal = () => {
    setData({ columns: [], rows: [] });
    setSearchText("");
    setHighlightedRowIndex(0);
    settextdata("Update Type Maintenance");
  ...
  const handleBlurRVC = (e) => {
    ...
    setTimeout(() => {
      const selectedItem = dataa.rows.find((item) => item.FTypCod === value);
      ...
      if (selectedItem) {
        setFormData({
          ...formData,
          AccountCodeform: selectedItem.FTypCod,
          Descriptionform: selectedItem.FTypDsc,
          Status: selectedItem.FTypSts,
  ...
  const handleInputChange = (e) => {
    ...
    if (name === "AccountCodeform") {
      ...
      const selectedItem = dataa.rows.find(
        (item) => item.FTypCod === formatToThreeDigits(formattedValue)
      );
  ...
  const filteredRows =
    ...
    data.rows.filter(
      (row) =>
        (row.FTypCod &&
          row.FTypCod.toLowerCase().includes(searchText.toLowerCase())) ||
        (row.FTypDsc &&
          row.FTypDsc.toLowerCase().includes(searchText.toLowerCase()))
    );
                        ...
                        onKeyDown={(e) => {
                          if (e.key === "Enter") {
                            ...
                            if (dataa.rows && dataa.rows.length > 0) {
                              const selectedItem = dataa.rows.find(
                                (item) => item.FTypCod === upperCaseValue
                              );
...
export default Type_Maintenance;
